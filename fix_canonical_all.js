...
// 1. apply precise_canonical_fix.js replacements
...
const oldActionSegment = `/** Verb-first form for compound actions (kebab segment \u2192 camelCase with verb first) */
function actionSegmentToCamel(segment: string): string {
  const parts = segment.split('-');
  const verbIndex = parts.findIndex(p => STANDARD_VERBS.has(p.toLowerCase()));
  if (verbIndex > 0) {
    // move verb to front
    const verb = parts.splice(verbIndex, 1)[0];
    parts.unshift(verb);
  }
  return toCamelCase(parts.join('-'));
}`;
content = content.replace(oldActionSegment, '');
